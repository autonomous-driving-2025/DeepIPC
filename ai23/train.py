...
import time
import numpy as np
from torch import device, torch
...
def train(data_loader, model, config, writer, cur_epoch, device, optimizer, params_lw, optimizer_lw):
    ...
    for data in data_loader:
        ...
        for i in range(0, config.seq_len): #append data untuk input sequence
            ...
            # print('pt_cloud_xs shape:', data['pt_cloud_xs'][i].shape)
            # print('pt_cloud_xs: ', pt_cloud_xs)
...
def main():
    ...
    if not os.path.exists(config.logdir+"/trainval_log.csv"):
        ...
        #set optima lw baru
        latest_lw = [
            float(log_trainval['lw_ss'][-1:]), 
            float(log_trainval['lw_wp'][-1:]), 
            float(log_trainval['lw_str'][-1:]), 
            float(log_trainval['lw_thr'][-1:]), 
            # float(log_trainval['lw_brk'][-1:]), 
            # float(log_trainval['lw_redl'][-1:]), 
            # float(log_trainval['lw_stops'][-1:])
            ]
